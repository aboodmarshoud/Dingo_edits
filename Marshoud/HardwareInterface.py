...
class HardwareInterface():
    def __init__(self, link):
        ...
        self.physical_calibration_offsets = np.array([[7, 4, -3, -3], [110, 131, 84, 6], [-53, 31, -64, -20]])
        self.link=link
        ...
        for leg_index in range(4):
            for axis_index in range(3):
                ...
                self.pi.set_PWM_range(pin, 250)  # 255 by default, adjust if needed
...
## HERE ##
    ...
    def joint_angles_to_servo_angles(self,joint_angles):
        ...
        for leg in range(4):
            ...
            THETA0 = lower_leg_angle_to_servo_angle(self.link, np.pi/2-THETA2, THETA3 + np.pi/2) # TODO draw a diagram to describe this transformatin from IK frame to LINK analysis frame

            #Adding offset from IK angle definition to servo angle definition, and conversion to degrees
            self.servo_angles[0,leg] = np.degrees( joint_angles[0,leg] ) # servo zero is same as IK zero
            self.servo_angles[1,leg] = np.degrees( THETA2              ) # servo zero is same as IK zero
            self.servo_angles[2,leg] = np.degrees( np.pi/2 + np.pi-THETA0) # servo zero is different to IK zero
...
def lower_leg_angle_to_servo_angle(link, THETA2, THETA3):
    ...
    # Triangle section
    CDH = 3/2*np.pi - THETA2 - GDE - link.EDC
